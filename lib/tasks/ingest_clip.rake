...
namespace :ingest_clip do
  ...
  task setup: :environment do 
    ...
    @storage_machine_id = ENV['storage_machine_id'] 
    @capture_machine_id = ENV['capture_machine_id'] 


    puts "Stream #{@stream_id} Clip: #{@clip_file}"
    if @stream_id.nil? || @clip_file.nil? || @storage_machine_id.nil? || @capture_machine_id.nil?
      puts "Usage: rake ingest_clip:ingest stream_id=<stream-id> clilp_file=<clip-file> storage_machine_id=<storage-machine-id> capture_machine_id=<caputre-machine-id>"
  ...
  task :ingest => [ :setup ] do
    begin
      ...
      rel_StreamFolder = "streams/#{@stream_id}"
      @localDataFolder = "#{serverFilePath}/#{rel_StreamFolder}"
      rel_CaptureFolder = "#{rel_StreamFolder}/#{captureModel.id}"
  ...
  task :cleanup do
    if File.exists? @tmp_folder_root
      FileUtils.rm_r @tmp_folder_root
    end
    puts("#{@localDataFolder} #{ File.exists? @localDataFolder}")
    if @is_production && (File.exists? @localDataFolder)
      puts "remove local data foler"
      FileUtils.rm_r @localDataFolder
    end
  end
